...
const ProfileOverview = () => {
  ...
  useEffect(() => {
    if (authenticatedUserDetails?.status === UserStatus.APPROVED) {
      ...
      } else if (vendor?.vendorVerificationStatus === VendorVerificationStatus.INCOMPLETE) {
        router.push(appRoutes.vendor.onboarding.root);
      }
    }
  ...
  if (status === UserStatus.APPROVED && vendor) {
    ...
    if (vendor.vendorVerificationStatus === VendorVerificationStatus.PENDING) {
      return (
        <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center bg-linear-to-br from-sky-50 via-white to-blue-50 p-4">
          <div className="flex max-w-md flex-col items-center justify-center space-y-4 text-center">
            ...
            <h2 className="text-2xl font-semibold">Your vendor profile is complete!</h2>
            <p className="text-muted-foreground">
              Wait for our team to review your information. We will notify you once your profile is
